/**
 * Shared assertions against the real "NM Custom Plan Intake Form" sample —
 * used by both the HTML-parser and PDF-parser verification scripts, since
 * both must produce the same result from the same underlying document.
 */
import type { CustomPlanIntakeParsed } from '../lib/import/customPlanIntakeShared';

export function runChecks(parsed: CustomPlanIntakeParsed): number {
  let failures = 0;
  ...
  for (const needle of [
    ...
    '10', // hours/week (also matches other numbers, just presence-checked)
  ]) {
